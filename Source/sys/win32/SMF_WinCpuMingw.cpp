...
void startRecordingTimeStamp(double &start){
	asm volatile( "mov  %0, ebx\n"
	"xor eax, eax\n"
	::"m"(SMF::MATH::saved_ebx) :);
	start = getClockTicks();
...
void stopRecordingTimStamp(double &end){
	asm volatile("xor eax, eax\n":::);
	end = 	getClockTicks();
...
bool resultado;
CFPU::fpu_HasDazSupport(resultado);
return resultado;
