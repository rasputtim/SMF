/*
  SMF - Salvatho Math Fabric  (https://sourceforge.net/projects/smfabric/)
...
============
*/
void VPCALL CSIMD_SSE2::cmpLT( sf_u8 *dst, const sf_u8 bitNum, const float *src0, const float constant, const int count ) {
	int i, cnt, pre, post;
	float *aligned;
	sf_m128 xmm0, xmm1;
	__m128i xmm0i;
...
============
*/
void VPCALL CSIMD_SSE2::matX_LowerTriangularsolve( const CMatXD &L, float *x, const float *b, const int n, int skip ) {
	int nc;
	const float *lptr;
...
============
*/
void VPCALL CSIMD_SSE2::matX_LowerTriangularsolveTranspose( const CMatXD &L, float *x, const float *b, const int n ) {
	int nc;
	const float *lptr;
